...
if [[ "2.7" =~ "$python" ]]; then
    conda install --yes anaconda-client jinja2
        conda convert -p all ~/miniconda2/conda-bld/linux-64/oompy-dev*.tar.bz2 -o ~/miniconda2/conda-bld/
    anaconda -t ${BINSTAR_TOKEN}  upload  --force -u omnia -p oompy-dev $HOME/miniconda2/conda-bld/*/oompy-dev*.tar.bz2
fi
